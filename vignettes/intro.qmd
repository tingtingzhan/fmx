...
#| fig-width: 4
ggplot() + 
  geom_function(fun = qfmx, args = list(dist = e1), n = 501L, xlim = c(0,1)) +
...
ggplot() + 
 ...
 scale_x_continuous(labels = percent, limits = c(.01, .99)) +
...
## Convert package **`mixtools`** returns to `'fmx'`

```{r}
#| message: false
m = faithful$waiting |> normalmixEM(k = 2)
class(m)
```
...
```{r}
m1 = smsn.mix(x, nu = 3, g = 3, family = 'Skew.normal', calc.im = FALSE)
m1 |> as.fmx(data = x)
```

```{r}
m2 = smsn.mix(x, nu = 3, g = 3, family = 'Normal', calc.im = FALSE)
m2 |> as.fmx(data = x)
```

```{r}
m3 = smsn.mix(x, nu = 3, g = 3, family = 'Skew.t', calc.im = FALSE)
m3 |> as.fmx(data = x)
```

```{r}
m4 = smsn.mix(x, nu = 3, g = 3, family = 't', calc.im = FALSE)
